                              ...
                              size_t dns_reply_max_len, dns_server_handle_t h)
{
    ...
    if (reply_len > (int)dns_reply_max_len) return -1;
...
static void dns_server_task(void *pvParameters)
{
    ...
    while (handle->started) {
        ...
        if (sock < 0) { ESP_LOGE(TAG, "socket failed: errno %d", errno); break; }

        int err = bind(sock, (struct sockaddr *)&dest_addr, sizeof(dest_addr));
        if (err < 0) { ESP_LOGE(TAG, "bind failed: errno %d", errno); close(sock); break; }
