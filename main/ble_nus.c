...
#define BLE_DEVICE_NAME "MouseKeepAlive"
...
/* RX: central writes to ESP32 (6E400002) */
static const ble_uuid128_t nus_rx_chr_uuid =
    BLE_UUID128_INIT(0x9e, 0xca, 0xdc, 0x24, 0x0e, 0xe5, 0xa9, 0xe0,
                     0x93, 0xf3, 0xa3, 0xb5, 0x02, 0x00, 0x40, 0x6e);

/* TX: ESP32 notifies central (6E400003) */
                          ...
                          struct ble_gatt_access_ctxt *ctxt, void *arg)
{
    if (ctxt->op == BLE_GATT_ACCESS_OP_WRITE_CHR) {
        ...
        uint8_t buf[om_len];
        ble_hs_mbuf_to_flat(ctxt->om, buf, om_len, NULL);
        if (s_rx_cb) s_rx_cb(buf, om_len);
        return 0;
    }
...
static void nus_advertise(void)
{
    ...
    rc = ble_gap_adv_start(s_own_addr_type, NULL, BLE_HS_FOREVER, &adv, nus_gap_event, NULL);
...
esp_err_t ble_nus_init(ble_nus_rx_cb_t rx_cb)
{
    ...
    if (ret != ESP_OK) { ESP_LOGE(TAG, "nimble_port_init: %d", ret); return ret; }

    ble_hs_cfg.reset_cb = nus_on_reset;
    ble_hs_cfg.sync_cb  = nus_on_sync;

    int rc = ble_gatts_count_cfg(nus_gatt_svcs);
    if (rc != 0) { ESP_LOGE(TAG, "gatts_count_cfg: %d", rc); return ESP_FAIL; }

    rc = ble_gatts_add_svcs(nus_gatt_svcs);
    if (rc != 0) { ESP_LOGE(TAG, "gatts_add_svcs: %d", rc); return ESP_FAIL; }
    ...
    ESP_LOGI(TAG, "BLE NUS initialized (device: %s)", BLE_DEVICE_NAME);
    return ESP_OK;
}
